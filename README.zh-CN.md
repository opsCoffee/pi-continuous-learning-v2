...
- `PI_PROJECT_DIR=/path/to/project` 可显式指定当前项目根目录
- 同时也兼容 `CLAUDE_PROJECT_DIR`，便于在 ECC 风格环境下复用
...
1. 当前会话活跃模型
2. `observer.model`，格式为 `provider/id`
3. `~/.pi/agent/settings.json` 中的默认模型
...
- 在生成前执行 quality gate，检查：
  ...
  - 现有 instincts 重叠
  - `MEMORY.md` 重叠
- overlap 判断现在会同时参考现有 skill 的标题、章节、列表动作、section 正文、段落和更长正文，而不只是元信息
...
本地 overlap 检查现在已经提升到更深的全文结构比对，因此对“前半段看起来不同，但后半段正文高度相似”的 skill 也更容易正确吸收而不是重复保存。
